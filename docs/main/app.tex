...
0--3, 3--6, 6--11, and 11+ years (Table~\ref{tab:data.demo}).   %SM: in re-reading this, I think providing the data in Table A2 would be good in the main manuscript rather than appendix/supplementary material. then can end this section with an intepretation of the inference made from Fig 3i and 3ii especially?
...
% JK: this feels a bit unfinished but I'm not sure what else to comment on SM: how about an intepretation line here re: so what with a concrete example? that usually goes in discussion, but I think would be good for reader here - i.e. the take away so-what without the math. "That is, when applied to aggregate data from one survey, we esitmated that X% had been engaged in X years of sex work, etc...(i.e. intepret Figure 3ii without the math/equations?)
% ==============================================================================
\subsection{Aggregate data from a global review}
Noting the limitations of previously considered methods to estimate $\E[x]$, %SM: to which previous limitations is this referring to? suggest we remove sentences like this for ease of reading.
Next, we applied the model to re-synthesize %SM: what does "re-synthesize" mean? - how does this lead from the objectives as currently written? 
...
%SM: what are the points you want to make? because I found this next part hard to follow and got lost in what the point of doing this application was. i.e. I agree. Overall, I think we have to know exactly which points we want to make, and then concrete examples and also sign-post things ahead of time.
% for example, if we want to discuss families, then we have to a priori say that in the objectives and applications that we are going to do this. So I suggest work backwards from the key points/messages, and then structure the para? 
...
Across all distribution families considered,  %SM: it seems like this is the key message? I would value (as a reader/user) some concrete examples / numbers to make this more "grounded" (same suggestion as for above application).
our estimates are consistently shorter than those of \cite{Fazito2012},
with best-fitting families ranging
from 1.5x shorter in Africa to 4x shorter in Latin America. %SM: what might this mean to a AJE reader? i would provide some estimates X-years vs. Y-years. [also, there is a lot of push-back on using terms like Africa or Latin America... given heterogeneity within regions. is there a reason we cannnot write which country, or is that becuase we grouped our results by region too? sorry if I'm forgetting but am not clear on the rationale for why groupiing - based on the introduction or the start of this application section]
\begin{figure}
