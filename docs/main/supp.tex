\part{Supplementary Content}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Parametric equations}
% ------------------------------------------------------------------------------
\paragraph{Distribution families}
We considered 6 distribution families for $p(x)$ in our analysis
(Table~\ref{tab:fams}).
\begin{table}[h]
  \centering
  \caption{Distribution families considered,
    using common parameter notation ($\aa,\bb$)}
  \label{tab:fams}
  \input{tab.fams}
\end{table}
\clearpage % HACK
% ------------------------------------------------------------------------------
\paragraph{Likelihoods functions}
Under the model for $p(z|s)$, we define the likelihoods
for empirical estimates of the mean $\sE_i[z|s]$ and proportions $\sP_i[\azb|s]$
from a given survey with sample size $N_i$ as, respectively:
the Gamma-equivalent of Normal sampling error for the mean (since $\sE > 0$), and
the Beta-approximation of the Binomial distribution (for differentiability):
\begin{alignat}{1}
  \sE_i[z|s;N_i] &\sim \txn{Gamma}(\aa = N_i~/~\CV_i^2[z|s],~~
                                   \bb = N_i~/~\CV_i^2[z|s]~/~\E[z|s])
  \label{eq:lE} \\
  \sP_i[\azb|s;N_i] &\sim \txn{Beta}(\aa = N_i~   \P_i[\azb|s],~~
                                     \bb = N_i~(1-\P_i[\azb|s]))
  \label{eq:lP}
\end{alignat}
\par
To validate these likelihood functions,
we simulated 10,000 stochastic samples of $z$
with sample sizes $N_i = \{30, 100, 300, 1000\}$
given known $p(z|s)$ under $\E[x] = 5, \CV[x] = \{0.5, 1, 1.5\}$,
and all 6 distribution families.
We then compared the distribution of stochastic estimates of
$\sE[z|s;N_i]$ and $\sP[\azb|s;N_i]$ from each sample
with the analytic likelihood functions: \eqrefa{eq:lE}{eq:lP}.
For proportions, we considered
$\P[\lzu01]$, $\P[\lzu15]$, and $\P[\lzu59]$, arbitrarily.
\par
Figures~\ref{fig:val.lf.mean}--\ref{fig:val.lf.p59} illustrate these comparisons,
where we observe good agreement between the
stochastic and analytic distributions across all conditions.
Note that stochastic distributions for $\sP[\azb|s;N_i]$
are discrete (more visible at low $N_i$) because
there are a finite number simulated respondents
whose durations can fall in the specified interval $[a,b]$.
\foreach \slug/\stat in {%
  mean/$\E[z|s]$,%
  p01/$\P[\lzu01|s]$,%
  p15/$\P[\lzu15|s]$,%
  p59/$\P[\lzu59|s]$}{
\begin{figure}
  \centering\includegraphics[width=\textwidth]{val.lf.\slug.pdf}
  \caption{Comparison of analytic likelihood functions
    and distribution of 10,000 stochastic estimates of \textbf{\stat},
    for different simulated survey sizes $N_i$ and
    latent distribution families for $p(x)$}
  \label{fig:val.lf.\slug}
\end{figure}}
\clearpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Illustrative analytic results}
Figure~\ref{fig:toy.distr} repeats Figure~\ref{fig:toy.distr.1}
for 5 distribution families considered (Table~\ref{tab:fams}) --- \ie
showing distributions of total ($x$) and censored ($z$) durations
among the source, active, and sampled populations for each family,
all having $\E[x] = 5$ and $\CV[x] = \{0.5, 1, 1.5\}$.
We omit the Exponential family as it is
a special case of both Gamma and Weibull with $\CV[x] = 1$.
Figure~\ref{fig:toy.est} further illustrates
performance of different $\E[x]$ estimators considered in \cite{Fazito2012}
across a range of $\CV[x]$ and the same 5 distribution families.
\vspace{-2cm} % HACK
\begin{figure}
  \centering\includegraphics[width=\textwidth]{toy.distr}
  \caption{Illustrative distributions of total and censored durations
    among the source, active, and sampled populations for
    different $x$ distribution families (columns) and
    3 coefficients of variation $\CV[x]$ (rows)}
  \label{fig:toy.distr}
  \floatfoot{Note: true $\E[x] = 5$;
    probability of being sampled modelled per \eqref{eq:p(s|z)} with $\pp = 0.5, \tt = 1$.}
\end{figure}
\begin{figure}
  \centering\includegraphics[width=\textwidth]{toy.est}
  % TODO: investigate truncation issues @ CV->2
  \caption{Performance of $\E[x]$ estimators considered in \cite{Fazito2012}
    across a range of $\CV[x]$ and distribution families}
  \label{fig:toy.est}
  \floatfoot{Note: true $\E[x] = 5$;
    probability of being sampled modelled per \eqref{eq:p(s|z)} with $\pp = 0.5, \tt = 1$;
    means and medians for censored data ($z$) are doubled per \cite{Fazito2012}.}
\end{figure}
\clearpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Application}
% ==============================================================================
\subsection{Aggregate data from a single survey}
Figure~\ref{fig:demo.pars} illustrates the posterior distributions
inferred by fitting our Bayesian model to
the RDS-adjusted sex work duration data reported in \cite{Baral2014}.
The best-fitting distribution family, based on overall log-likelihood, was Gamma (purple).
\begin{figure}[h]
  \includegraphics[width=\textwidth]{demo.pars}
  \caption{Posterior distributions of model parameters and summary statistics
    when fitting to the RDS-adjusted Eswatini data (Table~\ref{tab:data.demo})}
  \label{fig:demo.pars}
\end{figure}
\clearpage % HACK
% ==============================================================================
\subsection{Aggregate data from a global review}
\label{s.app.meta}
% ------------------------------------------------------------------------------
\paragraph{Data}
The manually extracted data from Table~S2 in \cite{Fazito2012}
are provided as supplementary file \texttt{Fazito2012.csv}.
We extracted all reported duration means $\E[z|s]$ and
interval proportions $\sP[\azb|s]$ (including medians and maximum durations),
but dropped from our analyses for simplicity:
standard deviations, minimum durations, and durations reported as "greater/less than X".
We also dropped estimates from the following 9 sources
due to the female sex worker (FSW) population definition
(it's not clear whether these studies were included
 in the quantitative syntheses of \cite{Fazito2012}):
\begin{itemize}[parsep=0pt]
  \item Anzala (1995) Kenya: FSW in a cohort who acquired HIV
  \item Ward (2006) UK: FSW who retired and were followed-up
  \item Pineda (1995) Spain: FSW working for at least 21 months
  \item Strathdee (2008) Mexico: duplicate of Loza (2010) Mexico
  \item MOH (2003) Cambodia: not clearly FSW (\eg karaoke workers)
  \item MOH (2007) Cambodia: not clearly FSW (\eg karaoke workers)
  \item Gu (2009) China: FSW who inject drugs
  \item Silverman (2006) India: women trafficked into selling sex
  \item Simoes (1993) India: women incarcerated for selling sex
\end{itemize}
\par
Before synthesizing the extracted data via our Bayesian model,
we also adjusted the effective sample sizes for each study $N_i$
to reduce the influence of very large studies via:
\begin{equation}
  N_i^* = \frac{1}{\frac{1}{N_0}+\frac{1}{N_i}}
  ,\qquad N_0 = 1000
  \label{eq:n.adj}
\end{equation}
This adjustment loosely approximates a random effects approach
by weighting each study more equally.
% ------------------------------------------------------------------------------
\paragraph{Results}
Figures \ref{fig:meta.Africa}--\ref{fig:meta.LatAm} illustrate
the posterior distributions of model parameters, summary statistics,
and cumulative/density duration distributions and means versus the data
for female sex workers reviewed in \cite{Fazito2012}.
Figure~\ref{fig:meta.forest.all} then summarizes estimates of
mean total duration in sex work among the source population $\E[x]$
obtained via our model versus those reported in \cite{Fazito2012},
across all distribution families considered.
\clearpage
% TODO: fix skumar convergence issues
\foreach \slug/\region in \regions{
  \begin{figure}[h]
    \begin{subfigure}{\textwidth}
      \includegraphics[width=\textwidth]{meta.pars.\slug.pdf}
      \caption{Model parameter and summary statistics}
      \label{fig:pars.\slug}
    \end{subfigure}
    \begin{subfigure}{\textwidth}
      \includegraphics[width=\textwidth]{meta.distrs.\slug.pdf}
      \caption{Modelled cumulative (CDF) and density (PDF) distributions and means
        versus empirical data}
      \label{fig:distrs.\slug}
    \end{subfigure}
    \caption{Model posteriors when fitting to
    empirical aggregate female sex worker duration data
    reviewed in \cite{Fazito2012} for \textbf{\region}}
    \label{fig:meta.\slug}
    \floatfoot{\distrfoot}
  \end{figure}}
\begin{figure}
  \includegraphics[width=\textwidth]{meta.forest.all}
  \caption{Forest plot of estimated mean total duration
    among female sex workers $\E[x]$
    versus regional synthesis estimates in \cite{Fazito2012}}
  \label{fig:meta.forest.all}
  \floatfoot{Synthesis methodology in \cite{Fazito2012} was unclear,
    and regional estimates were variably reported as points or ranges;
    see Table~\ref{tab:meth} for method definitions.
    Ns: number of unique surveys contributing to the estimate;
    BF: best-fitting family per region.}
\end{figure}
